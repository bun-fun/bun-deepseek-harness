...
- **TODO:** this backend routes SQLite through the `dsh-sqlite-runtime` facade. If a cordis database service (`cordis/db` / a `@cordisjs` SQL driver plugin) is adopted, route through that instead of holding a `DatabaseSync` here — the contract surface (`SessionPersistence`) would not change, only the storage driver.
